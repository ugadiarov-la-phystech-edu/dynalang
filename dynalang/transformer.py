...
Re-implements transformer.py (root of repo) as nj.Module subclasses so the
...
class TransformerEncoderLayer(nj.Module):
  ...
  def __call__(
    ...
    if self._norm_first:
      ...
      ff = jax.nn.relu(
          self.get('linear1', Linear, self._dim_feedforward)(normed))
      ...
      ff = jax.nn.relu(self.get('linear1', Linear, self._dim_feedforward)(src))
      ff = _dropout(ff, self._dropout, training)
      ff = self.get('linear2', Linear, self._d_model)(ff)
...
class TransformerDecoderLayer(nj.Module):
  ...
  def __call__(
    ...
    if self._norm_first:
      ...
      ff = jax.nn.relu(self.get('linear1', Linear, self._dim_feedforward)(tgt))
      ff = _dropout(ff, self._dropout, training)
      ff = self.get('linear2', Linear, self._d_model)(ff)
...
class TransformerEncoder(nj.Module):
  ...
  def __init__(
      self, num_layers, d_model, nhead, dim_feedforward=2048,
  ...
  def __call__(
    ...
    for i in range(self._num_layers):
      x = self.get(
          ...
          self._d_model, self._nhead, self._dim_feedforward,
...
class Transformer(nj.Module):
  ...
  def __init__(
      ...
      dim_feedforward=2048, dropout=0.1, norm_first=False):
    self._d_model = d_model
    self._nhead = nhead
  ...
  def __call__(
    ...
    memory = self.get(
        ...
        self._dim_feedforward, self._dropout, self._norm_first, True)(
            src, mask=src_mask,
            src_key_padding_mask=src_key_padding_mask,
            training=training)
    output = self.get(
