...
class CometOutput:
  def __init__(
      self, name, config, fps, pattern=r'.*', video_dir='/tmp',
      keep_videos=False):
    self._pattern = re.compile(pattern)
    self._fps = fps
    self._video_dir = os.path.abspath(os.path.expanduser(video_dir))
    self._keep_videos = keep_videos
  ...
  def __call__(self, summaries):
    ...
    for step, name, value in summaries:
      ...
      if isinstance(value, str):
        ...
        path = os.path.join(self._video_dir, f'{uuid.uuid4()}.mp4')
        try:
          clip.write_videofile(path)
          result = experiment.log_video(path, name=name, step=step)
          print(f'Comet video logged: {name}: {result}')
        finally:
          if not self._keep_videos and os.path.exists(path):
            os.remove(path)
