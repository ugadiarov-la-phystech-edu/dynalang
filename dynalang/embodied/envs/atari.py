...
class Atari(embodied.Env):
  ...
  def __init__(
    ...
    with self.LOCK:
      ...
      self.ale.setInt(b'random_seed', int(self.rng.integers(0, 2 ** 31)))
  ...
  @staticmethod
  def _normalize_lives(lives):
    if lives in ('unused', 'discount', 'reset'):
      return lives
    if lives in (False, None, 'none', 'False', 'false'):
      return 'unused'
    if lives in (True, 'True', 'true'):
      return 'discount'
    return lives
