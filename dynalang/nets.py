...
class MultiEncoder(nj.Module):
  ...
  def __init__(
    ...
    self.n_image_slots = 1 if (self.cnn_shapes and self.slot_based) else 0
    self.n_text_slots = 1 if (self.mlp_shapes and self.slot_based) else 0
    self.n_output_slots = self.n_object_slots + self.n_image_slots + self.n_text_slots
  ...
  def __call__(self, data, zero_mlp=False, zero_cnn=False):
    ...
    if 'text' in result and 'slot' in result:
    ...
    elif 'text' in result and 'image' in result:
  ...
  def __init__(
    ...
    if self.cnn_shapes:
      ...
      if cnn == 'resnet':
        ...
        raise NotImplementedError(cnn)
    if self.slot_image_shapes:
      slot_img_shape = tuple(self.slot_image_shapes['slot_image'][1:])
      if cnn == 'resnet':
