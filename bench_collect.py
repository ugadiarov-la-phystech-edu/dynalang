"""FPS / scaling benchmark for data collection with BatchEnv / BufferedBatchEnv.

For each environment (homegrid, langroom, vln) we collect a fixed number of
total env-steps (default 5000) through ``embodied.BatchEnv`` (or
``BufferedBatchEnv`` when ``--spare`` > 0) while sweeping the number of
parallel envs N. We measure the *total wall time to accumulate the target
number of steps* and keep increasing N until the time stops improving
(plateau).
...
Run (inside the conda env that has the envs installed):
    ...
    # Baseline BatchEnv
    python bench_collect.py --envs vln --sweep 1,2,4,8,16,32,64,128,252 \
        --max-envs 252 --no-early-stop --gpu 5 \
        --outdir bench_collect_out/vln_baseline

    # BufferedBatchEnv: one spare per active env (double buffering)
    python bench_collect.py --envs vln --spare-eq-n \
        --sweep 1,2,4,8,16,32,64,128,252 --max-envs 252 --no-early-stop \
        --gpu 5 --outdir bench_collect_out/vln_buffered

Outputs (in bench_collect_out/ by default):
    - results.csv               one row per (env, num_envs, spare); includes
                                  reset counts during timed collection
    - collect_time.png          collection time, build time, fps vs N
    - collect_resets.png        reset frequency during timed collection
...
Notes:
    ...
    - With ``--spare-eq-n``, each sweep point N spawns N active + N spare
      worker processes (2N total). Plan GPU/RAM accordingly.
"""
import argparse
...
_HAS_BUFFERED = hasattr(embodied, 'BufferedBatchEnv')
...
DEFAULT_SWEEP = [1, 2, 4, 8, 16, 24, 32, 48, 64, 128, 252]
...
def build_batch_env(config, amount, strategy, spare=0):
  ctor = bind(make_env, config)
  if strategy != 'none':
    ctor = bind(embodied.Parallel, ctor, strategy)
  if spare > 0:
    if not _HAS_BUFFERED:
      raise RuntimeError(
          'BufferedBatchEnv is not available on this branch; use --spare 0')
    envs = [ctor() for _ in range(amount + spare)]
    return embodied.BufferedBatchEnv(envs, amount, strategy != 'none')
...
def collect(env, target_steps, warmup=3, seed=0):
  ...
  reset_env_count = 0
  reset_driver_steps = 0
  while env_steps < target_steps:
    reset_n = int(np.sum(acts['reset']))
    reset_env_count += reset_n
    if reset_n > 0:
      reset_driver_steps += 1
    obs = env.step(acts)
    env_steps += n
    driver_steps += 1
    acts = policy(n)
  ...
  return {
      ...
      'reset_env_count': reset_env_count,
      'reset_driver_steps': reset_driver_steps,
      'reset_frac_env_steps': reset_env_count / env_steps,
      'reset_frac_driver_steps': reset_driver_steps / driver_steps,
      'resets_per_1000_env_steps': reset_env_count / env_steps * 1000,
...
def run_env_sweep(name, strategy, target, sweep, min_improve, repeats, warmup,
                  early_stop, spare=0, spare_eq_n=False):
  mode = 'BufferedBatchEnv' if spare or spare_eq_n else 'BatchEnv'
  print(f'\n=== {name}  ({mode}, strategy={strategy}, '
        f'target={target} env-steps) ===')
  print(f'{"N":>4}  {"spare":>5}  {"build(s)":>8}  {"collect(s)":>10}  '
        f'{"total(s)":>8}  {"fps":>9}  {"res/1k":>7}  {"vs prev":>8}')
  results = []
  prev = None
  for N in sweep:
    ...
    t_build = time.time()
    try:
      env = build_batch_env(config, N, strategy, spare=spare_n)
    except Exception as e:  # noqa: BLE001
      print(f'  !! could not build {name} with N={N} spare={spare_n}: '
            f'{type(e).__name__}: {e}')
      if not results:
    ...
    print(f'{N:>4}  {spare_n:>5}  {best["build_s"]:>8.3f}  '
          f'{best["time_per_target"]:>10.3f}  {best["total_per_target"]:>8.3f}  '
          f'{best["fps"]:>9.1f}  {best["resets_per_1000_env_steps"]:>7.1f}  '
          f'{imp_str:>8}')
...
def write_csv(all_results, path):
  cols = ['env', 'strategy', 'num_envs', 'spare', 'driver_steps', 'env_steps',
          'reset_env_count', 'reset_driver_steps', 'reset_frac_env_steps',
          'reset_frac_driver_steps', 'resets_per_1000_env_steps',
          'wall', 'build_s', 'time_per_target', 'total_per_target', 'fps']
  lines = [','.join(cols)]
...
def plot(all_results, target, path):
  ...
  for name in envs:
    ...
    spare = rs[0].get('spare', 0) if rs else 0
    label = f'{name} (spare-eq-n)' if spare else name
...
def plot_resets(all_results, path):
  import matplotlib
  matplotlib.use('Agg')
  import matplotlib.pyplot as plt
  ...
  for name in envs:
    ...
    spare = rs[0].get('spare', 0) if rs else 0
    label = f'{name} (spare-eq-n)' if spare else name
    axes[0].plot(ns, [r['resets_per_1000_env_steps'] for r in rs],
                 'o-', label=label)
    axes[1].plot(ns, [r['reset_frac_driver_steps'] for r in rs],
                 'o-', label=label)
  axes[0].set_xlabel('number of envs (N)')
  axes[0].set_ylabel('resets per 1000 env-steps')
  axes[0].set_title('Reset frequency during timed collection')
  axes[1].set_xlabel('number of envs (N)')
  axes[1].set_ylabel('fraction of driver steps with reset')
  axes[1].set_title('Driver steps with >=1 reset vs N')
  for ax in axes:
    ax.grid(alpha=0.3)
    ax.legend()
  fig.tight_layout()
...
def main():
  ...
  max_envs = args.max_envs or min(252, (os.cpu_count() or 8) * 2)
  if args.sweep:
  ...
  mode = 'buffered' if args.spare or args.spare_eq_n else 'batchenv'
  print(f'envs={env_names}  mode={mode}  strategy={args.strategy}  '
        f'target={args.target}  sweep={sweep}  '
        f'spare={args.spare}  spare_eq_n={args.spare_eq_n}  '
        f'early_stop={not args.no_early_stop}')
  ...
  for name in env_names:
    rs = run_env_sweep(
        ...
        spare=args.spare, spare_eq_n=args.spare_eq_n)
  ...
  plot_resets(all_results, outdir / 'collect_resets.png')
  ...
  for name in sorted({r['env'] for r in all_results}):
    ...
    print(f'  {name:10s}  N={best["num_envs"]:>3}  spare={best["spare"]:>3}  '
